...
export default function Chatbot() {
  ...
  return (
    <>
      {/* Chatbot Button */}
      <div className="fixed bottom-20 right-6 z-50 sm:bottom-20">
        {!isOpen && (
          <button
            onClick={() => setIsOpen(true)}
            className="bg-gradient-to-r from-fuchsia-500 to-violet-600 hover:from-fuchsia-600 hover:to-violet-700 text-white rounded-full p-4 shadow-lg hover:shadow-xl transition-all duration-300 group animate-bounce hover:animate-none"
          ...
          >
            ...
            <span className="absolute -top-8 right-0 bg-slate-900 text-white text-[10px] px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Pregúntame algo 💬</span>
          </button>
        )}
      </div>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-[360px] sm:w-96 h-[32rem] bg-slate-900 rounded-lg shadow-2xl border border-slate-700 z-50 flex flex-col">
          {/* Header */}
